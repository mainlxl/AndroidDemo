...
public class PasswordView extends View {
    ...
    private int borderColorHighLight;//当前正在输入的边框颜色
...
//            inputManager.showSoftInput(this, 0);
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        outAttrs.inputType = InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_FLAG_SIGNED | InputType.TYPE_NUMBER_FLAG_DECIMAL; //输入类型为数字
        return new BaseInputConnection(this, true) {
            ...
            public boolean commitText(CharSequence text, int newCursorPosition) {
                if (TextUtils.isDigitsOnly(text)) {
                    ...
                    String addText = add(text.toString());
    ...
    private void readAttribute(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mode = Mode.formMode(typedArray.getInteger(R.styleable.PasswordView_childmode_mode, Mode.UNDERLINE.getMode()));
            ...
            borderWidth = typedArray.getDimensionPixelSize(R.styleable.PasswordView_borderWidthValue, dp2px(3));
            borderColor = typedArray.getColor(R.styleable.PasswordView_borderColorValue, Color.BLACK);
            borderColorHighLight = typedArray.getColor(R.styleable.PasswordView_borderColorHighLightValue, Color.parseColor("#3C77FC"));
...
//        setOnKeyListener(new MyKeyListener());
        ...
        getViewTreeObserver().addOnPreDrawListener(new ViewTreeObserver.OnPreDrawListener() {
            ...
            public boolean onPreDraw() {
                postDelayed(() -> inputManager.showSoftInput(PasswordView.this, 0), 200);
...
//    }
    ...
    private void drawCipherText(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i < password.length; i++) {
            if (!TextUtils.isEmpty(password[i])) {
                ...
                if (cipherEnable) {
                    //没有开启明文显示，绘制密码密文
                    canvas.drawText(CIPHER_TEXT, (getPaddingLeft() + passwordSize / 2) + (passwordSize + passwordPadding) * i, getPaddingTop() + y, paint);
                } else {
                    //明文显示，直接绘制密码
                    canvas.drawText(password[i], (getPaddingLeft() + passwordSize / 2) + (passwordSize + passwordPadding) * i, getPaddingTop() + y, paint);
                }
            }
        }
    }
    ...
    private void drawCursor(Canvas canvas, Paint paint) {
        ...
        if (!isCursorShowing && isCursorEnable && !isInputComplete && hasFocus()) {
            ...
            // 终止点x = 起始点x
            // 终止点y = 起始点y + 光标高度
            canvas.drawLine((getPaddingLeft() + passwordSize / 2) + (passwordSize + passwordPadding) * cursorPosition, getPaddingTop() + (passwordSize - cursorHeight) / 2, (getPaddingLeft() + passwordSize / 2) + (passwordSize + passwordPadding) * cursorPosition, getPaddingTop() + (passwordSize + cursorHeight) / 2, paint);
        }
    }
    ...
    private void drawUnderLine(Canvas canvas, Paint paint) {
        int currentPosition = getInputPosition();
        //画笔初始化
        paint.setStrokeWidth(borderWidth);
        paint.setStyle(Paint.Style.FILL);
        for (int i = 0; i < passwordLength; i++) {
            if (i == currentPosition) {
                paint.setColor(borderColorHighLight);
            } else {
            ...
            canvas.drawLine(getPaddingLeft() + (passwordSize + passwordPadding) * i, getPaddingTop() + passwordSize, getPaddingLeft() + (passwordSize + passwordPadding) * i + passwordSize, getPaddingTop() + passwordSize, paint);
        }
    }
    ...
    /**
     * 获取当前输入的位置，最大为{@link #passwordLength}
     *
     * @return
     */
    private int getInputPosition() {
        int position = 0;
        while (position < password.length && !TextUtils.isEmpty(password[position])) {
            ++position;
        }
        return position;
    }
