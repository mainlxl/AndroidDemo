...
public abstract class QRCodeView extends FrameLayout implements Camera.PreviewCallback {
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        ...
        if (!mSpotAble || (mProcessDataTask != null && (mProcessDataTask.getStatus() == AsyncTask.Status.PENDING || mProcessDataTask.getStatus() == AsyncTask.Status.RUNNING))) {
            return;
        }
    ...
    private PointF transform(float originX, float originY, float cameraPreviewWidth, float cameraPreviewHeight, boolean isMirrorPreview, int statusBarHeight, final Rect scanBoxAreaRect) {
